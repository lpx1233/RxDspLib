function [output, ChnlCoeffs, costs] = mlseEqualize(InputSignal, TrainingSignal, ...
	...
	% using a TraceBackLen taps FFE with a learning rate of alpha and training epoch times.
	...
	TrainingSignalZP = [zeros(TraceBackLen - 1, 1); TrainingSignal];

	% Weights Initializing
	w = zeros(TraceBackLen, 1);
	w(TraceBackLen) = 1;

	% Training using LMS learning algorithm
	costs = zeros(epoch, 1);
	for n = 1 : epoch
		for i = 1 : length(TrainingSignalZP) - TraceBackLen + 1
			y(i) = w' * TrainingSignalZP(i : i + TraceBackLen - 1);
			w = w - alpha * (y(i) - InputSignal(i)) * TrainingSignalZP(i : i + TraceBackLen - 1);
			costs(n) = costs(n) + 0.5 * ((y(i) - InputSignal(i)) ^ 2);
		end
		% Record the cost/error of each epoch
		costs(n) = costs(n) / (length(TrainingSignalZP) - TraceBackLen + 1);
	end
